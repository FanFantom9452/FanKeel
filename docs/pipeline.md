...
flowchart TD
    ...
    D0["<b>where a ledger exists</b><br/>ledger.js ranges — one verifier per row, a task's or a fix's<br/><i>each pinned at both ends, which is not the same as disjoint</i><br/><i>the last paragraph says what it found: a row to drop, two rows</i><br/><i>that are one, a pair to send both of, a range nobody checked —</i><br/><i>or none of those, and they go out together</i>"]
