...
(`assets/station/station.js:1429`, `moved onto the page they narrow`) — and
...
`goneNote()` (`assets/station/station.js:1252`, `function goneNote(root)`) prints a
...
(`assets/station/station.js:1269`, `function registryNote(root)`) prints its
...
replaces sorted by it (`assets/station/station.js:1420`, `a sort key with no header is a sort nobody can reach`). `gather` still returns sessions ordered by
...
(`assets/station/station.js:1308`, `cleared ' + S.cleared + ' stale rows`); a
