...
(`assets/station/station.js:109`, `s.root !== f.project`) rather than a DOM
...
`drawDetail()`'s own per-stage `w.to - w.from` (`assets/station/station.js:612`, `w.to - w.from`) — coloured by
...
`沒有分階段紀錄` (`assets/station/station.js:629`, `沒有分階段紀錄`) — a
...
`goneNote()` (`assets/station/station.js:440`, `function goneNote()`) prints a
...
(`assets/station/station.js:466`, `cleared ' + S.cleared + ' stale rows`); a
