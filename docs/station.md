...
outside this file
(`scripts/station.js:640`, `r.sessions = r.sessions.filter((s) => !hidden.has(s.project ? r.root + '/' + s.project : r.root));`).
...
(`scripts/station.js:741`, `hidden by station.hide`), but names none of
...
(`scripts/station.js:506`, `const known = model.registries.some(`), so a
