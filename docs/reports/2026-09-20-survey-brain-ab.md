...
source_of_truth: 兩次互動實跑（session 5121ea58、942566e9）的 transcript，與七次 headless 量測的直接輸出：`ab.sh`（`HEAD 315f58bb7fb5945e9df9c7f4783a03e356f3829e`）、`ab2.sh`（`f34f846f9dfe04607a82d014b73d606dcdc1533f`）、`ab3.sh`（`577f3e1d6fb7305cf632c65795895660e891ce61`）、`ab4.sh`（`357924cc2ecea2546785145df722e7b1d0af712a`）、`ab5.sh` 與 `ab6.sh`（`d3d69135cbb3b07ad1e0e0b72e1e86238694b89b`）、`ab7.sh`（`0e32d9ab6d9b05dce2542a3a9c2db66f65ee4ac9`），每一次的 HEAD 與指令都在各自的 provenance.txt；全部複製在 [evidence/2026-09-20-survey-brain-ab/](evidence/2026-09-20-survey-brain-ab/)。本頁每一個數字都從那裡的檔案來，本頁不會重新產生
---

# survey 交給 Opus 大腦：新舊模式的量測 — 2026-09-20

**新模式跑得通：Sonnet 主控派 Opus 大腦、大腦派 Sonnet reader、使用者看到的關卡題目就是交接檔裡的原文。一開始在這個 survey 題目上它慢 9–10 倍、貴 4.0–5.6 倍（大腦每次派四個 reader）；拿掉 reader 是 6.5–9.0 倍與 2.5–2.8 倍。之後四次改大腦——指令串進一個 Bash、引用的行用 `sed -n` 讀、`effort: medium`、交接檔守字數上限——最後一組的組內花費是 1.05× 與 0.97×、時間 1.44× 與 1.57×。但同一段時間裡舊模式自己也變貴了（掃描面從 566 個檔長到 656），所以這個打平是兩頭一起靠近，只有組內可比。主控在關卡時的 context 只有不派 reader 的那一組比舊模式小（少 1,093 與 8,529），其餘每一組都比較大。**
...
`0e32d9a` 在 brief 加一行：output 規則的字數上限算的是這個檔案，引用的區塊與 gate 除外。`ab7.sh` 同時修掉一個測試面的問題——每一臂的 `task.js start` 改成從自己的 `--plugin-dir` 跑，`start` 印的 `<plugin>` 就和 hook 注入的一致。第 9 節兩個停下來問的主控卡的正是這個不一致；這一組四臂都正常派出了大腦。兩件事同時改，所以下面的數字不能單獨歸給那一行。

| arm | 時間（秒） | 花費 | output | subagents | 主 session 在最後一輪的 context |
|---|---|---|---|---|---|
| old12 | 93 | $0.93 | 7,383 | 無 | 67,183 |
| nor10 | 134 | $0.97 | 10,104 | 1 brain | 75,161 |
| old13 | 82 | $0.82 | 6,263 | 無 | 65,184 |
| nor11 | 129 | $0.80 | 9,990 | 1 brain | 73,711 |
...
- **字數上限沒有讓交接檔變短。** nor10 的交接檔 7,721 bytes、nor11 6,424（`ab7/handoff-nor10.md`、`ab7/handoff-nor11.md`）；第 7 節是 6,376 與 9,202，第 9 節的 nor7 是 7,444。Write 估 3,377 與 2,758 tokens，也在原本的範圍裡。那一行沒有生效。
- **花費接近打平，但兩頭都在動。** 組內 1.05× 與 0.97×，第 5 節是 2.5–2.8 倍。大腦便宜了：$0.67 與 $0.47（第 7 節 $0.81、$1.14），thinking 估 2,478 與 1,983（跑 `high` 時是 5,513 與 7,310）。同一段時間舊模式卻變貴：old12、old13 花 $0.93 與 $0.82、用 14 與 13 次工具，第 7 節的 old7、old8 是 $0.45 與 $0.49、各 4 次。掃描面仍在長（`ab7/scan-at-0e32d9a.txt` 是 656 個檔）。所以這個打平是兩頭一起靠近的結果，跨批不能直接比，組內可以。
- **時間與 context 沒有打平。** 時間仍是 1.44× 與 1.57×；主控在關卡時的 context 比同組舊模式多 7,978 與 8,527。
- **n=2**，理由同第 4 節。
