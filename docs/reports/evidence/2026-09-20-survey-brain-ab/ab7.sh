#!/usr/bin/env bash
# ab5.sh again, after 0e32d9a bound the output rule's word count to the handoff file
# (report §10). One harness change: each arm runs `task.js start` from its own
# --plugin-dir, so the <plugin> that start prints and the one the hook injects agree —
# in ab5 and ab6 they did not, and two nor controllers stopped to ask about it instead
# of dispatching. The old arms point at the working tree, so nothing about them moves.
#
...
arm () {
  ...
  local c1=(claude -p "Run this command and report its output, nothing else: node $plugin/scripts/task.js start --session $U --task \"$TASK\" --route survey,design" --session-id "$U" --model "$model" --output-format json "${common[@]}")
  logcmd "${c1[@]}" >> "$LOG"
...
arm old12 opus   false "$REPO"
arm nor10 sonnet true  "$COPY"
arm old13 opus   false "$REPO"
arm nor11 sonnet true  "$COPY"
