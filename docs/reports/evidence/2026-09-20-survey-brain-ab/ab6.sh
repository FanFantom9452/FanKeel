#!/usr/bin/env bash
# The pair ab5 lost: its nor8 controller stopped to ask about the scratchpad plugin path
# instead of dispatching, and its old10 ran long. Same setup as ab5, nothing else changed,
# one pair only (report §9).
#
...
arm old11 opus   false "$REPO"
arm nor9  sonnet true  "$COPY"
