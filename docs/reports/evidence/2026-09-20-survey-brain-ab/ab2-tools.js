...
function tally(file) {
    ...
    for (const line of fs.readFileSync(file, 'utf8').split('\n')) {
        ...
        for (const b of o.message.content || []) if (b.type === 'tool_use') { tools++; by[b.name] = (by[b.name] || 0) + 1; if (b.name === 'Bash') { commands.push(String(b.input.command).replace(/\n/g, ' ').slice(0, 160)); } }
    }
    return 'tool calls ' + tools + ' (' + Object.entries(by).map(([k, n]) => k + ' ' + n).join(', ') + '), first to last timestamp ' + Math.round((last - first) / 1000) + 's';
}
let commands = [];
