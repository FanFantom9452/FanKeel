#!/usr/bin/env bash
# ab4.sh again, after d3d6913 set the stage agent to `effort: medium` (report §9). The
# copy is taken from HEAD, so the nor arms carry it and both brief lines; the old arms
# stay at the session's effort. The arms alternate old, nor, old, nor.
#
...
arm old9  opus   false "$REPO"
arm nor7  sonnet true  "$COPY"
arm old10 opus   false "$REPO"
arm nor8  sonnet true  "$COPY"
