#!/usr/bin/env bash
# ab3.sh again, after 357924c told the stage agent to read the lines it cites with
# `sed -n` in one Bash call (report §7). The copy is taken from HEAD, so the nor arms
# carry that line and the batching one; the arms alternate old, nor, old, nor.
#
...
arm old7 opus   false "$REPO"
arm nor5 sonnet true  "$COPY"
arm old8 opus   false "$REPO"
arm nor6 sonnet true  "$COPY"
