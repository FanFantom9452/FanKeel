#!/usr/bin/env bash
# The control the first batch lacked (report §5): the new mode with the brain's
# fan-out off, against the old mode, arms alternating old, nor, old, nor in one
# loop (a context arm drifts between batches).
#
...
arm old3 opus   false "$REPO"
arm nor1 sonnet true  "$COPY"
arm old4 opus   false "$REPO"
arm nor2 sonnet true  "$COPY"
