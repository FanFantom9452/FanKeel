...
這句話能撐到哪裡，要說清楚。「無效名字、`exit=0`、`stderr` 空」證明的是**沒有驗證、也沒有生效**；它**不足以**證明這個鍵「根本沒有被讀進來」——一個讀了鍵、查不到樣式、然後靜靜放棄的實作，會產生一模一樣的輸出。兩者在這一次跑裡分不開，所以本頁只主張前者。

它跟前四次的關係也是一樣的分寸：這一次跑讓「`--settings` 這條路上的 `outputStyle` 不生效」多了一個獨立的觀察，**它不是前四次的機制解釋**，只是同一個結論的第二個證據。
...
- **計畫指定的儀器根本沒有被試過。** Task 10 寫的是「在 `.claude/settings.json` 裡設 `outputStyle`」，而這個檔案在本 repo 不存在——本頁改用 `claude -p --settings <檔>`，那是**另一個管道**，不是同一個。為了跑一次探針而在 repo 裡生一個 settings 檔再刪掉，是比較差的形狀，所以做了這個替換；但替換本身就是一個沒被試過的變因，寫在這裡而不是留在 ledger 裡。
