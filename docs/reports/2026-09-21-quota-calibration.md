...
source_of_truth: 五份輸出都由 `docs/reports/evidence/2026-09-21-quota-calibration/` 底下同名腳本產生，各自帶著產生它的那個 sha — `windows-at-43daef5.txt`（全機 427 份頂層 transcript 的逐分段表）在 `43daef5`；`basis-at-93208e2.txt`（兩次捕捉的對照、基準本身的兩側不確定，與 `quotaLimits`）在 `93208e2`；`drift-at-512710e.txt`（66 個 session 的重分配）與 `sonnet-at-512710e.txt`（Sonnet 主控那一趟）在 `512710e`；`roundings-at-d375d34.txt`（頁面上哪些數字不是逐字的，逐項分類）在 `d375d34`。三個 sha 是三次審查之後的修改：basis 補上第二份 transcript，drift 與 sonnet 把「文件裡提到指令」與「指令真的跑過」分開。`windows.js` 自 `43daef5` 起一個位元都沒動。兩次 statusline 捕捉分別是 `evidence/2026-09-21-long-task-projection/quota-capture-260920T163041Z.txt` 與 `evidence/2026-09-21-quota-calibration/quota-capture-260920T203515Z.txt`。費率表那次讀取的引文在 `pricing-read-260921.txt`。
---
...
美元區間有交集，token 區間完全不相交。**「這支表在數未加權的 token」可以丟掉了。**

**成因不是模型比例。** 這一句原本寫「第二段混進了便宜的 Sonnet」，是錯的：照
`windows-at-43daef5.txt` block 4，Opus 的 token 佔比兩段是 67.3% 與 68.5%，Sonnet
的佔比其實從 32.7% 降到 31.5%。兩段的混合單價確實差 1.692 倍（$0.975／百萬 對
$0.576／百萬），但那來自 token 的**種類**組成（快取讀取對寫入對輸出），而這些輸出
只拆到模型、拆不到種類，所以成因在這裡是未量測的。

**但不能因此說它「在數錢」。** API 等價美元本身就是「token 乘上每個分量的費率」，
所以「按錢走」和「按價格加權的 token 走」在這份資料上是同一件事，分不開 —— 兩段能
排除的只有**不加權**那一種。任何權重接近費率表的計量方式都同樣合得上。真正要分開
它們，得找兩個成本相同而 token 組成不同的視窗，這裡沒有。

照 $7.64–$9.17：一整個 5 小時視窗約 **$764–$917**，而這個換算本身就吃了上面那個
「按錢走」的假設。
...
兩個讀數共用不了同一個速率，差**至少 4.7 倍**。被排除的是一個很具體的模型：
**從 `resets_at` 往前推七天當起點、以單一固定速率累積**。這比「不是流量的函數」窄，
而窄的那個才是這兩個點能說的。

因為「計帳起點不是 `resets_at` 往前七天」本身就把兩個讀數調和掉了：把起點挪晚到
讀數 A 附近，A 的累積接近 0（落在 `[0, 0.5)`）、B 的累積就是 A..B 的 $91.68，
速率約 $30.56 一點，正好落在 `basis-at-*.txt` block 3 單獨從 A..B 算出來的
$26.19–$45.84 裡。那一版仍然是「記到的流量的函數」，只是量的起點不同。上一版把
這一段寫成斷言，卻在下一句自己列出這個可能 —— 兩句互相矛盾。
...
**而「所以是下限」是錯的，推翻它的就是上面那張表的第一列。** 讀數 A 那一刻，
transcript 算出的 $12.7472 **高於** Claude Code 自己的 $12.5610，`+1.48%`——同一個
方法在那裡是高估。所以偏差兩個方向都有：A 是 `+1.48%`、B 是 `-10.69%`。

$91.68 因此**不是下限**。它的不確定度就是上面那兩個觀測本身：`+1.48%` 與
`-10.69%`，n=2，兩個方向都出現過，跨幅約 12 個百分點而且**不對稱**。這裡不寫成
「±10%」——兩次觀測給不出一個對稱的帶，寫成對稱的就是把 1.48% 也講成 10%。上面每一個
$／點都繼承這一項，而區間裡那個 ±0.5 只涵蓋整數讀數的捨入，不涵蓋它。

（`basis.js` 原本只查 Opus 那一份 transcript，而 $91.68 是兩個 session 相加，所以
那個論證只對一半成立。審查抓到，現在兩份都查——然後同一輪審查指出整個論證的方向也是
錯的。）
...
兩種分桶的總額完全相等，0 個 mismatch。**但那個相等大半是構造出來的**：同一串
request 在同一條連續時間軸上重新切一次，只要切點單調、每個 request 仍落進恰好一個
視窗，總額就必然守恆。所以它排除的是腳本的計數錯誤（漏掉、重複、切點逆序），**不是**
「新的切法比舊的更對」的證據。後者靠的是 §3 那個紅綠，不是這個等式。
...
- **兩段的 $/點各帶一個不對稱、n=2 的不確定**：transcript 基準對 Claude Code 自己的
  數字，一次高 `1.48%`、一次低 `10.69%`。兩個觀測不構成一個帶，所以沒有一個數字可以
  寫成「±多少」。
- **13.9% 那組數字沒有誤差帶。** `drift.js` 的比對式還有一個殘餘缺陷（一道把整串
  呼叫連 `--session` 一起引用的指令仍會被當成呼叫），註解裡寫明它兩個方向都可能錯，
  但沒有在 66 個 session 上量過它的發生率。所以 66／280／13.9%／7.2%／183.1 分鐘
  都是點值，不是區間。
