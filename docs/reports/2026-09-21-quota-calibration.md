...
source_of_truth: 五份輸出都由 `docs/reports/evidence/2026-09-21-quota-calibration/` 底下同名腳本產生，各自帶著產生它的那個 sha — `windows-at-43daef5.txt`（全機 427 份頂層 transcript 的逐分段表）在 `43daef5`；`basis-at-93208e2.txt`（兩次捕捉的對照、基準本身的兩側不確定，與 `quotaLimits`）在 `93208e2`；`drift-at-512710e.txt`（66 個 session 的重分配）與 `sonnet-at-512710e.txt`（Sonnet 主控那一趟）在 `512710e`；`roundings-at-76dd71b.txt`（頁面上哪些數字不是逐字的，逐項分類）在 `76dd71b`。三個 sha 是三次審查之後的修改：basis 補上第二份 transcript，drift 與 sonnet 把「文件裡提到指令」與「指令真的跑過」分開。`windows.js` 自 `43daef5` 起一個位元都沒動。兩次 statusline 捕捉分別是 `evidence/2026-09-21-long-task-projection/quota-capture-260920T163041Z.txt` 與 `evidence/2026-09-21-quota-calibration/quota-capture-260920T203515Z.txt`。費率表那次讀取的引文在 `pricing-read-260921.txt`。
---
...
$91.68 因此**不是下限**，是一個帶著大約 ±10% 兩側不確定的數字，而上面每一個
$／點都繼承它。區間裡那個 ±0.5 只涵蓋整數讀數的捨入，不涵蓋這一項。
...
- **兩段的 $/點各帶約 ±10% 的兩側不確定**，因為 transcript 基準與 Claude Code 自己
  的數字在兩次捕捉上一次高 1.48%、一次低 10.69%，而只有兩次觀測。
