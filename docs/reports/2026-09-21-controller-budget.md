...
**這一節站不穩的地方有兩個，不是一個，而且它們吃到的數字不一樣。** 第一個是 n = 2，只吃到兩個：`34` 與 `11.3` 是拿第 4 節的 $3,056.00 除出來的，跟 22 與 7.3 一樣薄；`$87.39` 與 57.4% 不是，它們只用到 block 2 與 block 4，跟額度讀數無關。第二個是「token 量不變」，它吃到全部四個，而且沒有被量過。
...
- **不量 fankeel 自己的注入。** 每站的注入區塊上限 2,400 字元、實測 2,254–2,396（`tests/render.test.js:527` 與一次 `render` 實測）。它相對於一輪要重讀的整份 context 顯然很小，但這一頁沒有量那個比值，所以也不給那個比值一個數字。
