...
| Why the four `## Needs a decision` entries — station's not-shown projects, the gate's option labels and summary, how the station opens, and guard's `agent_type` — each came down to one change, and the one structural fact the first three lean on: `flatten()` is the one place every session enters the page | [plans/2026-09-14-todo-four-design.md](plans/2026-09-14-todo-four-design.md) — *design-intent, 繁體中文* |
| The seven tasks that close them: `station.hide` filtered at `flatten()` and nowhere else, the gate's kept option labels and its home-page cell, `agent_id` alongside `agent_type` in the guard, the `/fankeel-station` skill and the page's own dead-server poll, and this reference page caught up | [plans/2026-09-14-todo-four.md](plans/2026-09-14-todo-four.md) — *design-intent, 繁體中文* |
