...
| The first cut of that: `survey` handed to an Opus stage agent behind `stage.agents`, its brief carrying the stage's rules, its report and gate a file under `.fankeel/build/`, the gate filled from that file by `hooks/gate.js` | [plans/2026-09-19-survey-brain-design.md](plans/2026-09-19-survey-brain-design.md) — *design-intent, 繁體中文* |
| The seven tasks that build it: the `updatedInput` probe first, a handoff module, the controller's block and the stage agent's brief, the gate hook, the agent file, the documents, and a measurement against the old mode last | [plans/2026-09-19-survey-brain.md](plans/2026-09-19-survey-brain.md) — *design-intent, 繁體中文* |
