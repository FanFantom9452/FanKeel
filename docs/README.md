...
| Why the six entries the 09-18 menu offered are routed by the stage each belongs to rather than flattened into six build tasks, and why storing every gate option's wording closes a statistic that was already losing its denominator | [plans/2026-09-18-todo-six-design.md](plans/2026-09-18-todo-six-design.md) — *design-intent, 繁體中文* |
| The five tasks: two correction lines on a decision page, one restatement turned into a link, the `labels` field on the `gates` record, `gateSummary()`'s fallback onto it, and the two reference pages that follow | [plans/2026-09-18-todo-six.md](plans/2026-09-18-todo-six.md) — *design-intent, 繁體中文* |
