...
| `verify` | one row's claim over its pinned range — a task's or a fix round's — minus the rows the report's last paragraph says are not independent | `scripts/ledger.js --plan <f> ranges` |
