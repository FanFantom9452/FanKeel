...
| `verify` | one row's pinned range — a task's or a fix round's — as qualified by the report's last paragraph, which says what to send where rows are not independent, or could not be checked | `scripts/ledger.js --plan <f> ranges` |
