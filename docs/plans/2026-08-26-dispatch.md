...
> **Corrected 2026-08-26, in the fourth whole-branch review.** Two of the
> constraints above carry numbers that have since moved, and the cap that binds
> a new rule is not either of the ones named. The assertions still hold:
> `rulesFor(<stage>).join('\n').length < 2000` and the brief
> under 1400. The measurements do not — `survey` is 1690, `plan` 1788, `build`
> 1799, and the brief 823 as `tests/brief.test.js` measures it, with the
> `ALWAYS` block still 665 of each stage. What actually stops a rule being added
> is `tests/render.test.js`: the **whole injection** must stay under **2400**
> characters measured against a 59-character reference plugin root, and there
> `survey` sits at 2378 and `build` at 2374. Under that cap a rule is bought by
> displacing one, which is what splitting survey’s dispatch arm into a dispatch
> and a report did.

> **Corrected 2026-08-27, when the dispatch disclosure moved into `ALWAYS`.**
> Those measurements have moved again and the block above is a snapshot of the
> day it was written, not of the code. `ALWAYS` is 693; `survey` 1692, `plan`
> and `build` 1816 each; under the 2400 cap `survey` 2380, `plan` 2381,
> `audit` 2385 and `build` 2391 — nine characters spare on the binding stage,
> not the twenty-six this block implies. Read the diagnostics
> `tests/render.test.js` prints rather than any number written down here.
