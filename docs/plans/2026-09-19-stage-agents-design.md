...
- build 站內部怎麼派工留到後面那一刀。站 agent 開不了 `Workflow`（每個 subagent 都拿不到），
  所以 build 站在 brain 裡用 Agent 分回合派、一回合最多四個；由 script 產生 workflow 這條被 [2026-09-21-controlled-stations-design.md](../archive/2026-09-21-controlled-stations-design.md) 取代，留在 TODO 的 Waiting。
