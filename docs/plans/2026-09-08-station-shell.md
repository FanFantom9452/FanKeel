...
4. **`tests/source.test.js` — every exported name is imported by something.** It reads `module.exports = { … }` out of every tracked non-test `.js` and fails on a name nothing imports; a name reached only by a test counts as used. This reaches `assets/station/station.js`, so every name in its export block needs an importer, and `SCRIPT` must leave `lib/station.js`'s export block when it leaves the file.
