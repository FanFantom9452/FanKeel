...
- [ ] ~~**Step 3: 跑 `ask`。** 把 settings.json 裡的 `none` 改成 `ask`，關掉那個 `claude` 再重開（hook 清單在 process 啟動時就定了），重複 Step 2。~~ **沒跑，2026-09-20 的 verify 補記。** Step 4 的判準是「兩種都行時取 `none`」，所以 `none` 一通過，`GATE_DECISION` 就定了：`ask` 不管是哪個結果都改不動它。當時的 evidence 把這一步記成跑過了，這一行是更正。
