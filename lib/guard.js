...
function reasonFor(rel, holders) {
    ...
    lines.push('down without taking the task, and `task.js adopt <session-id>` still takes');
    lines.push('it back. To go back to warnings only, remove `guard` from this session’s entry.');
    return lines.join('\n');
}
