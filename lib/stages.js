...
const STAGES = [
    {
        ...
        rules: [
            ...
            'A capped scan is re-run with `--all`; a truncated walk needs `--root`, which no flag replaces; a `skipped:` line names what no flag opens. Dispatch readers over those, when the reading is wide, or when nothing matched at all: several in one response, one lens each. Never spend a round asking permission to read.',
