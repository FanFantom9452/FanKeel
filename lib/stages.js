...
const STAGES = [
    {
        ...
        rules: [
            ...
            'Scope from the tree before the first term. A capped scan re-runs with `--all`; a truncated walk needs `--root`, which no flag replaces. A `skipped:` line goes to one reader; its counts are reported. Dispatch when the reading is wide, or when nothing matched at all: several in one response, one lens each, with their count and model. Never ask permission.',
