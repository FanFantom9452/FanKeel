...
// The survey rule names a script, and its path is only knowable at run time.
...
function rulesFor(stage, subs) {
    ...
    const rules = found ? ALWAYS.concat(found.rules) : ALWAYS.slice();
    const survey = subs && subs.survey;
    if (!survey) return rules;
    return rules.map((r) => r.split(SURVEY_TOKEN).join(survey));
}

module.exports = { ALWAYS, STAGES, NAMES, SURVEY_TOKEN, byName, nextStage, rulesFor };
