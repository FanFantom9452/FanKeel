...
// Whether a stated `Done when` actually ends the gate loop, or only renames it is
// untested: `ALWAYS[0]`'s `or none` is the clause that would tell, and it lives in
// the model loop, where no test in this repository reaches it.
const ALWAYS = [
...
//
// Empty rather than removed: `rulesFor`'s `on(ALWAYS_WHEN)` and the mechanism
// it exercises stay exactly as written, and the export stays for whatever
// already reads it.
const ALWAYS_WHEN = [];
...
const INIT = [
    ...
    'Read `TODO.md` at the root if there is one. Its headings are the clustering: `## Ready` is one task for the whole section, and more than one bullet needs `plan` on the route; `## Needs a decision` offers what `orient`\'s `todo:` lists, one option each, the rest through Other; `## Waiting` is one option when `orient` marks any `due`; any other heading, or none, means clustering by hand — never one option per bullet; no `TODO.md` means guessing from the commits.',
...
const STAGES = [
    {
        ...
        rules: [
            ...
            '`fankeel` itself: run its scripts, not `<plugin>`\'s.',
            'Read whatever documents this area. If it disagrees with the code, say so now.',
            'State the ask in one line and have it confirmed.',
            'Scope from the tree before the first term. A capped scan re-runs with `--all`; a truncated walk needs `--root`. A `skipped:` line’s paths go to one reader; its counts are only reported. Wide reading or no match: one `fankeel-reader` per lens, one workflow, every path:line checked before it returns.',
            'Read the fankeel-survey skill on entry: ratchet the class with task.js route.',
            'Output: one line per finding, then the question. Under 120 words.',
        ],
        budget: 2400,
        template: [
    ...
    },
    {
        ...
        rules: [
            ...
            'Name what would prove it done \u2014 the test that fails now and passes after. "Make it work" is not a criterion.',
            'Check the approach against .fankeel/map.md before presenting it. Name the page, or say none was found.',
            'Two or more rows sharing no file and feeding nothing to each other are independent work: put `plan` on the route with `task.js route`.',
    ...
    },
    {
        ...
        rules: [
            ...
            'A task is the smallest unit carrying its own test cycle. Fold setup and docs into the task needing them.',
            ...
            'Every `## Task N:` carries `**Files:**`, `**Interfaces:**` and a `**Dispatch:**` line \u2014 `implementer, <model>` or `in-session`; the profile\'s `dispatch.floor` is the floor, and anything above it names why on that line.',
            'Read the fankeel-plan skill on entry: Read:, a fence names its file, ## Coverage.',
            'Output: one line per task as `N. name \u2014 its files`, then the question. Under 100 words; the file is the output.',
        ],
        budget: 2400,
        template: [
    ...
    },
    {
        ...
        rules: [
            'Run the tests and quote them. "Should work" is not a result.',
...
// One substitution, three callers: a stage's rules, `init`'s, and the
// controller's, which passes its own tokens.
function substitute(rules, subs, tokens) {
    if (!subs) return rules;
    const map = tokens || TOKENS;
    let out = rules;
    for (const key of Object.keys(map)) {
        const value = subs[key];
        if (!value) continue;
        out = out.map((r) => r.split(map[key]).join(value));
    }
    return out;
}
...
// The controller's block, for a stage handed to a stage agent. For a stage on
// `stage.agents`'s list, `lib/render.js` injects this in place of the stage's
// own rules and shape: the session holding the gate dispatches, relays a path
// and asks, and the stage's rules reach the stage agent through `renderBrief`
// instead. docs/archive/2026-09-19-survey-brain-design.md §2.
//
// Its tokens are its own rather than more `TOKENS`: they are filled only here,
// and a stage rule that used one would ship it raw.
//
// `advance` rather than a literal `stage {{NEXT}}`: the ordinary gate's option
// one already handles a route that ends here — the ordinary rule's `{{NEXT}}`
// falls back to "standing the task down" — and the controller's option one has
// to make the same choice rather than always naming a next stage that may not
// exist. The caller fills it with `'stage ' + next` or `'down'`.
const CONTROL_TOKENS = { handoff: '{{HANDOFF}}', answer: '{{ANSWER}}', session: '{{SESSION}}', advance: '{{ADVANCE}}' };
// Which stages are controlled is no longer a constant here: `lib/profile.js`'s
// `stage.agents` parses to an array of canonical stage names — `false` none,
// `true` `['survey']` for backward compatibility, `all` every stage, or a
// comma list — and `controlling`/`controlFor` read it from the caller's
// `values` rather than from a list only this file could change.
// The one thing a build agent asks of its controller: it may not commit, so it
...
const COMMIT_RULE = 'If it returns `commit <file>` instead of a report path, run `node {{COMMIT}} "<file>"` and SendMessage it what that printed, exactly. No gate, nothing else; wait for it to return again.';
const controlRules = (stage) => [
    ...
    'Option one: `node {{TASK}} {{ADVANCE}} --session {{SESSION}}`. The pause: `node {{TASK}} next --from-gate --session {{SESSION}}`.',
...
// `values['stage.agents']` is normally the array `lib/profile.js` parses it
// to. A bare `true` is read the same way `parseValue` reads it — survey
// alone — for a `values` object built by hand rather than through `read`.
function controlling(stage, values) {
    const raw = values && values['stage.agents'];
    const list = Array.isArray(raw) ? raw : (raw === true ? ['survey'] : []);
    return list.includes(String(stage || '').trim().toLowerCase());
}

function controlFor(stage, values, subs) {
    const name = String(stage || '').trim().toLowerCase();
    if (!controlling(name, values)) return null;
    return { rules: substitute(substitute(controlRules(name), subs), subs, CONTROL_TOKENS), template: CONTROL_TEMPLATE };
}
...
// task's own Dispatch line names — or, at verify, where no task names one, on
// the profile's `dispatch.floor`.
...
const STAGE_AGENTS = {
    ...
    verify: BRAIN_AGENTS.concat(['fankeel:fankeel-verifier', 'fankeel:fankeel-fixer', 'an implementer (`general-purpose`, on the `dispatch.floor` model, sent to apply a mutation, run the test and restore the file)']),
};
