...
const STAGES = [
    {
        ...
        rules: [
            ...
            'Scope from the tree before the first term. A capped scan re-runs with `--all`; a truncated walk needs `--root`, which no flag lifts. A `skipped:` line’s paths go to one reader; its counts are only reported. Dispatch when the reading is wide, or when nothing matched at all: several in one response, one lens each, with count and model. Never ask permission.',
