...
// already reads it. The judge rule itself moved into the four stages that ask
// the user something mid-stage — survey, design, plan, build — as an entry in
// each one's own `when`, sized for the cap the way this array's first draft
// was not.
const ALWAYS_WHEN = [];

// `{{JUDGE}}` is the token; the rule using it lives on each stage's `when` at
// the key `judge.enabled`.
const JUDGE_RULE = { when: 'judge.enabled', text: 'An in-stage question goes to `fankeel-judge` once — brief, dispatch, `node {{JUDGE}} record` — never the gate.' };
