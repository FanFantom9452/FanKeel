...
const STAGES = [
    ...
    },
    {
        ...
        rules: [
            ...
            'Every `## Task N:` carries `**Files:**`, `**Interfaces:**` and a `**Dispatch:**` line \u2014 `implementer, <model>` or `in-session`; `sonnet` is the floor, and anything above it names why on that line.',
...
function controlledList(values) {
    const raw = values && values['stage.agents'];
    if (Array.isArray(raw)) return raw;
    return raw === true ? ['survey'] : [];
}

function controlling(stage, values) {
    return controlledList(values).includes(String(stage || '').trim().toLowerCase());
}
