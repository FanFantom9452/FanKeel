...
const STAGES = [
    ...
    },
    {
        ...
        rules: [
            'Run `node {{DOCS_CHECK}}` and quote it. It reports references that no longer resolve, never opinions — a dead path in a reference document is a bug, in an archived one it is history.',
            'Use /ponytail-audit for the code if it is installed, and say plainly if it is not. Documents are outside its scope either way.',
