...
const STAGES = [
    {
        ...
        rules: [
            ...
            'A capped scan is re-run with `--all` first — that costs nothing; a truncated walk needs `--root`, which no flag replaces. Dispatch readers when the reading is wide, or when nothing matched at all: several in one response so they run at once, one lens each. Never spend a round asking permission to read.',
