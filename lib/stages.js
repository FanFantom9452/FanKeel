...
const STAGES = [
    {
        ...
        rules: [
            ...
            'A capped scan is re-run with `--all`; a truncated walk needs `--root`, which no flag replaces; a `skipped:` line names files never opened. Dispatch readers when the reading is wide or nothing matched at all: several in one response, one lens each. Never spend a round asking permission to read.',
    ...
    },
    {
        ...
        rules: [
            ...
            'Every fortnight /fankeel-audit adds the deep pass: `node {{DOCS_AUDIT}}` and offers the cleanup. Alongside it: /ponytail-audit for code, knip or deptry for unused packages — say if one is not installed.',
