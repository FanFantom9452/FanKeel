...
// granted: `build` now sits 13 characters under that cap.
//
// That figure said 6 while the test's own diagnostic said 23, because it was
// written from the arithmetic of one change and never re-read against a run.
// Ten of the 23 bought `or none` in the rule below — which is the amount the
// stale number would have refused. Take the diagnostic, never this line.
const ALWAYS = [
...
const INIT = [
    ...
    'Read `TODO.md` at the root if there is one. Its headings are the clustering: `## Ready` is one task for the whole section, `## Needs a decision` is one task each, `## Waiting` stays out of the menu because nothing there can move today. Any other heading, or none, means clustering by hand — never one option per bullet. No `TODO.md` means guessing from the recent commits instead.',
...
const STAGES = [
    ...
    },
    {
        ...
        rules: [
            ...
            'Every task carries `**Files:**` and a `**Dispatch:**` line \u2014 `implementer, <model>` or `in-session`. `sonnet` is the floor and needs no argument; anything above it names on that same line what the task needs that transcription does not. A task without either is a plan failure.',
    ...
    },
    {
        ...
        rules: [
            ...
            'A coverage claim states its denominator — nine of twenty-one pages, not "the pages". survey names what it skipped; nothing made verify do the same.',
            'Before the question, one read-only adversary over the evidence: was each command run after the last edit, on the thing claimed, by a check that could have failed. Give it paths, never a paste, and ask only for the rows it defeats — every line it returns stays in this session for good.',
    ...
    },
    {
        ...
        rules: [
            ...
            'Run `node {{RESIDUE}}` and quote it. Untracked and unignored is a decision nobody made; a merged worktree is spent.',
            'Every fortnight /fankeel-audit adds the deep pass: `node {{DOCS_AUDIT}}` and offers the cleanup. {{PONYTAIL}}',
            'Before the question, one read-only adversary over the findings: was it run, on what, and could it have failed.',
    ...
    },
    {
        ...
        rules: [
            'Commit the reason, not the diff — it is already in the commit. Subject is why; body is the shipped list, one bullet each.',
            ...
            'Output: three lines and the shipped list — what landed, what it cost, what is still open — then the question. Not a tour of the diff.',
        ],
        template: [
            '<sha> <subject>',
            'shipped:',
