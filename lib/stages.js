...
const STAGES = [
    {
        ...
        rules: [
            ...
            'A capped scan is re-run with `--all`; a truncated walk needs `--root`, which no flag replaces. On a `skipped:` line one reader takes what it can open; what nothing opens is reported, not dispatched. Dispatch also when the reading is wide, or when nothing matched at all: several in one response, one lens each. Never ask permission to read.',
