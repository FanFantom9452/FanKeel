...
const STAGES = [
    {
        ...
        rules: [
            ...
            'A capped scan is re-run with `--all` first — that costs nothing; a truncated walk needs `--root`, which no flag replaces; a `skipped:` line names files never opened. Dispatch readers when the reading is wide, when the report says it did not cover something, or when nothing matched at all: several in one response so they run at once, one lens each. Never spend a round asking permission to read.',
    ...
    },
    {
        ...
        rules: [
            ...
            'From a plan (the fankeel-build skill has the loop): `node {{LEDGER}} --plan <f> show` first and never redo a task it lists complete. One reviewer per task, then `complete <n> "<what landed>"`. After a compaction trust it over memory.',
    ...
    },
    {
        ...
        rules: [
            ...
            'Run `node {{RESIDUE}}` and quote it. Untracked and unignored is a decision nobody made; a merged worktree is one already spent; an environment nothing can rebuild or run is weight nobody can use.',
            'Every fortnight add the deep pass: /fankeel-audit runs `node {{DOCS_AUDIT}}` and offers the cleanup. Other tools answer in the same breath — /ponytail-audit for code, knip or deptry for unused packages, whichever manifest is here — and say if it is not installed.',
...
module.exports = { ALWAYS, STAGES, NAMES, TOKENS, SURVEY_TOKEN, FULL_ROUTE, CLASSES, byName, nextStage, normaliseRoute, positionIn, routeForClass, classForRoute, rulesFor, templateFor };
