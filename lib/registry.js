...
// `stampEntry` as a write of its own, for a caller with nothing else to do in
// the same callback.
function enter(projectRoot, sessionId, stage) {
    if (typeof stage !== 'string' || !stage) return false;
    return update(projectRoot, sessionId, (data) => {
        data.updated = new Date().toISOString();
        stampEntry(data, stage, Date.parse(data.updated));
        return true;
    });
}
