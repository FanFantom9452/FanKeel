...
// written down. A map keyed by stage caps itself at the length of the route, so
// there is no constant here to keep beside MAX_NOTES.
function touch(projectRoot, sessionId, used) {
    return update(projectRoot, sessionId, (data) => {
        ...
        if (Number.isFinite(used) && used > 0 && data.stage) {
            ...
            burn[data.stage] = [Array.isArray(seen) ? seen[0] : used, used];
            data.burn = burn;
        }
        return true;
    });
