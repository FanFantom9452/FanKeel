...
// The nearest ancestor that already holds a `.fankeel/`, the way git finds
// `.git`. This is what lets one registry cover several projects: put it at the
// directory they share and every session opened inside any of them joins it,
...
function findStateRoot(start) {
    ...
    for (;;) {
        ...
        if (fs.existsSync(path.join(dir, '.fankeel'))) return dir;
