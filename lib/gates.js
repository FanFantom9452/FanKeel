...
// A question that was never answered keeps the null `answerOf` gives it.
    ...
    for (const e of Array.isArray(entries) ? entries : []) {
        ...
        for (const b of Array.isArray(e.message.content) ? e.message.content : []) {
            ...
            for (const q of ask.questions) {
                ...
                // `answerOf` returns null for a question the answers object
                // has no key for, and `clip` would turn that into `''` — a
                // value `lib/station.js`'s `gateSummary()` cannot tell from a
                // real answer, so it would count an unanswered gate as option
                // one losing. `lib/replay.js:125` keeps the null; so does this,
                // and the two sources then give that gate the same answer.
                const answer = answerOf(answers[q.question]);
                out.push({
                    at: ask.at,
