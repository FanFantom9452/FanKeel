...
//
// Pid reuse is the ceiling. `procStart` on each entry is the field that
...
// The same scan, before it is thrown away down to a set of ids. One caller needs
// more than membership: `task.js` refuses a `--session` no running process
// claims, and a refusal that printed only ids would be naming eight things
// nobody can tell apart. The directory each session was opened in is what makes
// that list readable, so it is carried through.
//
// `cwd` is the only extra field taken. The entries hold a dozen more and none of
// them has a reader.
//
