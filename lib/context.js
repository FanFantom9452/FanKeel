...
// move is not.
// `sessionId` is only spent on the compaction branch, and that is the whole
// design. The id is disclosed in the `init` block, which a session sees only
// while it has no task; once it owns one the block never repeats it, so the
// session that most needs it is the one that cannot get it — after a compaction
// the id is out of context and every `task.js` call needs `--session <id>`.
//
// The compaction line is already conditional on something having been dropped,
// which is exactly the case, so a session that has never compacted still has the
// id where it first saw it and pays nothing for this.
function contextLine(info, sessionId) {
    if (!info) return null;
    ...
    // Inline with the counts rather than a sentence of its own: an id is 36
    // characters before anything is said about it, and this line sits in the one
    // block whose whole discipline is that it still gets read to the end.
    const id = sessionId ? ', --session ' + sessionId : '';
    return 'context: ' + k(info.dropped) + ' tokens dropped to compaction so far'
        + (info.used ? ', ' + k(info.used) + ' in play now' : '') + id + '. '
        + (heavy ? 'Start a fresh session before the next one. ' : '')
        + carry;
}

// `k` is exported as `tokens` rather than renamed: one definition of how a token
// count is written, and callers outside this file get a name that says so.
module.exports = { inspect, contextLine, tokens: k, TAIL, BUSY };
