...
//
// The trigger for saying anything is that a compaction has happened at all.
// Compaction only happens when the window filled, so one is already proof this
// session reached its limit — which is the thing worth acting on, and the thing
// a percentage would only be a proxy for.
...
function contextLine(info) {
    if (!info || !info.dropped) return null;
    ...
    return 'context: ' + k(info.dropped) + ' tokens dropped to compaction so far'
        ...
        + 'A new terminal and /fankeel → Adopt carries this task over with its notes and its route.';
}

module.exports = { inspect, contextLine, readTail, usedFrom, droppedFrom, TAIL };
