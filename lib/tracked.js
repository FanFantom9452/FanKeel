...
function trackedFiles(root) {
    ...
    if (direct) return { files: direct, repos: [], walked: false, truncated: false, unlistable: 0 };

    const state = { files: [], repos: [], truncated: false, unlistable: 0 };
    walk(root, '', state);
    if (!state.files.length) return null;
    return { files: state.files, repos: state.repos, walked: true, truncated: state.truncated, unlistable: state.unlistable };
}
