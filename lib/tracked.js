...
function trackedFiles(root) {
    ...
    walk(root, '', state);
    // No files, and no files with three subtrees that could not be listed, are
    // not the same fact. Returning null for both reported the second as
    // "nothing readable under that root — no repository, and no files", which
    // is false and reaches no rule. The count goes back so the caller can say
    // what actually happened.
    if (!state.files.length && !state.unlistable) return null;
