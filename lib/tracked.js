...
function walk(root, rel, state) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            // same path to be told again. Only the walk's own leaves go in: the
            // git output spliced in above is a nested repository's list, and a
            // gitlink inside it is indistinguishable from a file without one.
            state.known.add(sub);
...
function trackedFiles(root, opts) {
    ...
    // `known` is empty rather than absent here: every one of these came from
    // git, and a gitlink in that list looks exactly like a file, so none of them
    // is known to be one.
    if (direct) return { files: direct, known: new Set(), repos: [], walked: false, truncated: false, unlistable: 0, skippedExt: 0 };
