...
(function(){
  ...
  // One section per group, in the same document order render() writes them —
  // every non-gone registry emits exactly one of each, nested together, so
  // the i-th section is the i-th group's own container. A page with no
  // sections at all (nothing in this file's own tests renders one) leaves
  // this empty and the loop below finds nothing to hide.
  ...
  function apply(){
    ...
    groups.forEach(function(g,gi){
      ...
      // rows div: whatever emptied it — project, state default or term —
      // takes the whole thing with it, and a row surviving any later pass
      // brings it back.
      if(sections[gi])sections[gi].hidden=visible===0;
    });
