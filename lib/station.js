...
function rememberRoots(configDir, registries, now) {
    ...
    for (const r of registries) {
        ...
        else if (old[r.root]) next[r.root] = old[r.root];
    }
...
function gather(opts) {
    ...
    return { generatedAt: new Date(now).toISOString(), configDir, pricesVerified: prices.verified, registries, scanStats: found.scanStats };
}
