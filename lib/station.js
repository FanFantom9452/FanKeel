...
#nav{flex:0 0 200px;display:flex;flex-direction:column;gap:2px;position:sticky;top:24px}
#nav a{display:block;padding:5px 8px;border-radius:5px;color:inherit;text-decoration:none;font-size:13px;line-height:1.3}
#nav a:hover{background:var(--line)}
#nav a[aria-current=true]{background:var(--line);font-weight:600}
#nav a.gone{color:var(--stale);font-style:italic}
#nav .n{display:block;color:var(--mute);font-size:11px}
...
//
// The spend series is the whole session's: `w.usd` is the parent's requests in
// that window plus its agents', so this curve ends where the row's cost cell
// adds up to and not at the parent's third of it.
//
...
//
// `note` is `row()`'s shortfall line, carried here because the legend is the
// one place on the row where the totals are already being named. It rides the
// no-chart branch too: a session with one burn sample still prints a stage
// table, and a notice about that table must not depend on a curve being drawn.
function chart(stages, note) {
    const withBurn = stages.filter((w) => w.burn);
    if (withBurn.length < 2) return `<p class="nochart">no burn recorded${note ? ' · ' + esc(note) : ''}</p>`;
    const t0 = stages[0].from;
    // The largest `to` of any stage, not the last element's. `seriesOf` orders
    // stages by when each was entered, so a session that re-enters an earlier
    // stage and ends there keeps its widest window in the middle of the array;
    // reading the last element scaled x to the shorter span and drew every
    // point past it outside the box — one measured at x=940 in a 320-wide
    // viewBox. The legend's width figure is this same number, so it stopped
    // understating the session's span with it.
    const t1 = stages.reduce((n, w) => (w.to > n ? w.to : n), stages[0].to);
...
//
// `spend` is one total, parent and agents together, and not two columns. This
// table is captioned as the figures the chart is drawn from, and the chart
// draws one spend line; a split here would print two numbers neither of which
// is the plotted one, and leave the reader adding up a column to check the
// curve. `spend[stage]` on disk keeps the two apart for anyone who needs them,
// and the row's own cost cell prints the session's split as `$X + $Y`.
...
function row(s, opts, root) {
    // The two halves of the cost cell as numbers, because the sort key below is
    // the same figure and used to be only the first of them: a row printing
    // `$0.83 + $1.39` carried `data-cost="0.83"` and the page's own descending
    // sort put it under a row showing less money.
    //
    // The sort key is rounded to the cent the cell prints, like every other
    // figure in this file. Nothing is lost: `SCRIPT`'s `num()` reads it back
    // through `parseFloat`, so `2.22` and `2.2199999999999998` sort alike, and
    // a cent is the finest distinction the page shows anywhere.
    const ownUsd = s.cost && s.cost.priced.length ? s.cost.usd : 0;
    const theirUsd = s.agentCost && s.agentCost.priced.length ? s.agentCost.usd : 0;
    const usd = s.cost && s.cost.priced.length ? '$' + ownUsd.toFixed(2) : '';
    const agentUsd = s.agentCost && s.agentCost.priced.length ? '$' + theirUsd.toFixed(2) : '';
    ...
    // The cost cell, the stage table and the curve are three views of one sum,
    // and `lib/usage.js` buckets a request into a stage by the timestamp on its
    // transcript line — a line carrying none is counted in the row's total and
    // lands in no bucket. Rather than invent a stage for money that has none,
    // the page says how much of its own total the stages below do not account
    // for. Unpriced stages are excluded from both sides: `costOf` leaves a
    // model it has no rate for out of `usd`, and such a stage is `null` here,
    // so neither side counts it and the notice does not fire on every one.
    // Only the direction that has a number to name — total above stages — and
    // only where a stage table is printed at all: with no stages the row shows
    // one total, and one total cannot disagree with itself.
    const staged = s.stages.reduce((n, w) => (w.usd === null ? n : n + w.usd), 0);
    const missing = ownUsd + theirUsd - staged;
    const note = s.stages.length && missing >= 0.01 ? '$' + missing.toFixed(2) + ' unaccounted' : '';
    return `<details class="s ${s.state}"`
        + ` data-updated="${s.updated || 0}"`
        + ` data-started="${Date.parse(s.started) || 0}"`
        + ` data-cost="${(ownUsd + theirUsd).toFixed(2)}"`
        ...
        // The registry this row belongs to, by root — what a nav entry's own
        // `data-project` names, so `SCRIPT`'s project scoping is one equality
        // check between the two, not a lookup through anything else the row
        // carries.
        + ` data-project="${esc(root)}"`
        + ` data-text="${esc(text)}"`
        + `><summary>`
        ...
        + chart(s.stages, note) + stageTable(s.stages)
        + `<dl>`
...
const BAR = (serve) => `<div class="bar">`
    ...
    // Unconditional, unlike auto-refresh below: `down` rows start hidden on
    // every page, served or static, so the control that brings them back is
    // not a serve-only convenience.
    + `<label><input type="checkbox" id="showDown"> show down</label>`
    + (serve ? `<label><input type="checkbox" id="auto"> auto-refresh</label>` : '')
    + `<span id="shown" class="meta"></span></div>`;

// The only script on the page, inline so `tests/station.test.js:93`'s assertion
...
//
// `nav` and `showDown` are read once by id and every new branch below is
// gated on the element existing, the way the auto-refresh branch already
// gates on `auto`. A page that ships neither — nothing in this file's own
// tests renders one — runs exactly the filter and sort this script always
// had; the two-pane page always renders both, so there the new behaviour is
// not optional.
const SCRIPT = `
(function(){
  var q=document.getElementById('q');
  var groups=[].slice.call(document.querySelectorAll('.rows'));
  // Paired to a group by data-project, the value both a section and every
  // row inside its group already carry — not by position, which would only
  // ever break if something else on the page started carrying either class.
  // A page with no sections at all (nothing in this file's own tests renders
  // one) leaves this empty and the loop below finds nothing to hide.
  var sections=[].slice.call(document.querySelectorAll('.registry'));
  ...
  function markCurrent(){
    navLinks.forEach(function(a){
      a.setAttribute('aria-current',(a.getAttribute('data-project')||'')===selected?'true':'false');
    });
  }
  function apply(){
    var term=(q.value||'').toLowerCase(),n=0,total=0;
    var all=showDown?showDown.checked:true;
    // Rows and visible-rows, tallied by data-project as the loop below meets
    // them, so a section can ask "did any row of mine ever exist" rather than
    // only "how many are showing" — the two answers a zero-session registry
    // and a registry every row of which got filtered out both give as
    // visible===0, but only the second was actually emptied by anything.
    var counts=Object.create(null);
    groups.forEach(function(g){
      var rows=[].slice.call(g.children);
      rows.forEach(function(r){
        total++;
        var text=r.getAttribute('data-text')||'',state=r.getAttribute('data-state')||'',proj=r.getAttribute('data-project')||'';
        var textHit=!term||text.indexOf(term)!==-1||state.indexOf(term)!==-1;
        var projHit=!selected||proj===selected;
        // down is a default, not a lock: a search term already reaching
        // past it (matched above, on the same data-state a bare "down"
        // matches) must still surface the row, so the state filter only
        // applies with no term typed at all.
        var stateHit=all||term||state!=='down';
        var hit=textHit&&projHit&&stateHit;
        r.hidden=!hit; if(hit)n++;
        var c=counts[proj]||(counts[proj]={rows:0,visible:0});
        c.rows++; if(hit)c.visible++;
      });
    ...
    // A registry is one section now, not a heading floating over an empty
    // rows div — but only when something actually emptied it. A registry
    // with no sessions at all was never emptied by the project filter, the
    // state default or a typed term, so under "all projects" it stays put;
    // selecting a different project still takes it with everything else.
    sections.forEach(function(s){
      var proj=s.getAttribute('data-project')||'';
      var c=counts[proj];
      var excluded=!!selected&&proj!==selected;
      var emptied=!!c&&c.rows>0&&c.visible===0;
      s.hidden=excluded||emptied;
    });
    shown.textContent=n===total?total+' shown':n+' of '+total+' shown';
  }
  q.addEventListener('input',apply);
  if(showDown)showDown.addEventListener('change',apply);
  if(nav){
    // Read once on load, the way a served page's own URL survives a reload;
    // written again on every click so the next load starts where this one
    // left off.
    var raw=(location.hash||'').replace(/^#/,'');
    if(raw){try{selected=decodeURIComponent(raw)}catch(e){selected=''}}
    markCurrent();
    navLinks.forEach(function(a){
      a.addEventListener('click',function(){
        selected=a.getAttribute('data-project')||'';
...
// The nav label for a root is the shortest tail of its path segments that no
// other root shares — one segment until two collide, then one more for both,
// and so on. Three roots on this machine end in `datapacks`; the naive last
// segment would show `datapacks` three times over. The full root always goes
// in `title=`, so a collision costs a reader nothing but the extra segment on
// screen. `guard` bounds what would otherwise be a loop with no fixed point:
// once every colliding root is already showing all of its segments, no
// further growth can separate them, and nothing here can make two literally
// identical roots differ — `discover()` already keeps roots unique.
function navLabels(registries) {
    const segsOf = (root) => String(root).split(/[\\/]+/).filter(Boolean);
    const segs = registries.map((r) => segsOf(r.root));
    const depth = registries.map(() => 1);
    const labelAt = (i) => segs[i].slice(-depth[i]).join('/');
    for (let guard = 0; guard < 50; guard++) {
        const labels = registries.map((_, i) => labelAt(i));
        const counts = new Map();
        for (const l of labels) counts.set(l, (counts.get(l) || 0) + 1);
        let grew = false;
        labels.forEach((l, i) => {
            if (counts.get(l) > 1 && depth[i] < segs[i].length) {
                depth[i] += 1;
                grew = true;
            }
        });
    ...
    const out = new Map();
    registries.forEach((r, i) => out.set(r.root, labelAt(i)));
    return out;
}

// One entry per registry, in the same order the right pane lists them, so the
// nav and the sections underneath never disagree about which registry is
// which. A gone registry keeps its place and its label — a page whose job is
// to show every registry must not make one vanish — but prints no counts: it
// has no sessions to count, and `r.sessions` is empty for it in `gather()`.
function navHtml(model, labels) {
    const entry = (r) => {
        if (r.gone) {
            return `<a href="#${encodeURIComponent(r.root)}" data-project="${esc(r.root)}"`
                + ` title="${esc(r.root)}" aria-current="false" class="gone">${esc(labels.get(r.root))} — gone</a>`;
        }
        const c = { live: 0, stale: 0, down: 0 };
        for (const s of r.sessions) c[s.state]++;
        return `<a href="#${encodeURIComponent(r.root)}" data-project="${esc(r.root)}"`
            + ` title="${esc(r.root)}" aria-current="false">${esc(labels.get(r.root))}`
            + `<span class="n">${c.live} live, ${c.stale} stale, ${c.down} down</span></a>`;
    };
    return `<a href="#" data-project="" aria-current="true">all projects</a>`
        + model.registries.map(entry).join('');
}

function render(model, opts) {
    opts = opts || {};
    const counts = tally(model);
    const nav = navHtml(model, navLabels(model.registries));
    ...
    for (const r of model.registries) {
        if (r.gone) {
            // A gone registry is a section too — just a shorter one, with no
            // meta line, no clear form and no rows div to wrap. It still
            // needs to be one, because it still has a nav entry: without a
            // section carrying its root, selecting it would exclude every
            // other section (none of them is its root) and leave nothing on
            // the page saying why the pane went blank.
            body += `<section class="registry" data-project="${esc(r.root)}">`
                + `<h2>${esc(r.root)} <span class="gone">— gone: no sessions/ here any more</span></h2>`
                + `</section>`;
            continue;
        }
        // A registry is one element from here down: heading, meta line, the
        // clear-stale form and the rows div all sit inside one `<section>`
        // carrying the root as `data-project`, the same value `row()` already
        // writes on every row inside it. `SCRIPT` hides this whole section,
        // not just the rows div, the moment none of its rows are visible —
        // whether that is project selection, the state default or a typed
        // term with no match here — so a registry a reader has filtered away
        // does not leave its heading behind over empty space.
        body += `<section class="registry" data-project="${esc(r.root)}">`
            + `<h2>${esc(r.root)}</h2>`
            + `<p class="meta">${r.sessions.length} sessions, ${r.unreadable} unreadable`
    ...
    return `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">`
        + `<title>fankeel station</title><style>${CSS}</style></head><body>`
        + `<div class="layout"><nav id="nav">${nav}</nav><div class="main">${body}</div></div>`
...
module.exports = { discover, gather, render, write, scanRoots, readRoots, rootsPath, rememberRoots, SCRIPT };
