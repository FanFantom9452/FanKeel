...
function rememberRoots(configDir, registries, now) {
    ...
    for (const r of registries) {
        ...
        // for, and the comment above is why it keeps a stamp — its old one if
        // it has one, or a fresh one the first time it is ever seen gone. A
        // gone root whose directory is not there at all is a scratch tree from
        // a test run — two landed on 2026-09-06 — and nothing will ever find
        // it again.
        else if (fs.existsSync(r.root)) next[r.root] = isRootRecord(old[r.root]) ? old[r.root] : new Date(now).toISOString();
    }
