...
// And a backstop on what actually gets printed, because `MAX_TREE` counts rows
// and a `│` holding a subtree open is not one. Fifty rows drew 66 lines on the
// worst of 51 real trees — Trovara, a ratio of 1.32, where the median is 1.00 —
// so a hundred bites none of them. Without it the block is bounded only by how
// many continuation lines somebody's tree happens to carry, which is no bound.
...
function layoutBlock(root, declared) {
    ...
    for (const name of candidates) {
        ...
        for (let i = first; i < lines.length; i++) {
            ...
            const room = rows < MAX_TREE && out.length < MAX_TREE_LINES;
            if (MEMBER.test(lines[i])) {
                gap = 0;
                if (isRow) total += 1;
                if (room) { if (isRow) rows += 1; out.push(lines[i]); }
                continue;
            }
            if (++gap > 2) break;
            if (room) out.push(lines[i]);
        }
...
function pagesByStatus(root) {
    ...
    for (const rel of markdownUnder(root)) {
        ...
        if (text === null) continue;
        // The front door is not a page in a tree, and `undeclared` reads as an
        // invitation to sign one — which for these two the project has already
        // ruled out: GitHub renders a frontmatter block on a README as a stray
        // table at the top of it, and `scripts/todo-check.js` re-verifies
        // TODO.md in full on every run, so a `last_verified` there is a date
        // somebody must remember to bump standing in for a check that already
        // runs. `scripts/docs-audit.js` has excluded them since it learned the
        // word; this did not, and the difference was age rather than intent —
        // `pagesByStatus` predates `isSignpost`. One word, two behaviours, is
        // the defect naming them apart was meant to close.
        //
        // Counted, not dropped: `documents:` is these buckets added up, so a
        // `continue` here would quietly subtract two from a line that claims to
        // be every markdown file. `roleOf` already grades a signpost
        // `reference`, and `current` is the bucket that is counted but never listed.
        if (docs.isSignpost(rel)) {
            out.current.push(rel);
            continue;
        }
        ...
        // A fixture is a test's own input and carries no contract by design —
        // `docs-check` skips its symbols and its scaffold's paths for the same
        // reason — so a bare one is not undeclared. `current` is the bucket
        // counted but never listed, which is where a page making no claim goes.
        if (!contract.declared && docs.roleOf(tree, rel) === 'fixture') {
            out.current.push(rel);
            continue;
        }
...
function buildMap(root) {
    ...
    lines.push('');
    if (!layout) {
        // A declaration narrows the search to one file, so the signposts are the
        // wrong thing to name when it comes back empty: a project whose README
        // holds a good tree and whose docs.json misspells the filename would be
        // told no tree was found in three files none of which were opened. A
        // confident sentence pointing the wrong way is what this section exists
        // to prevent, so it must not be how this section fails.
        const want = (declared.tree && declared.tree.layout) || null;
        if (want && want.file) {
            lines.push('no directory tree in ' + want.file
                + (want.heading ? ' under ' + want.heading : '')
                + ', which .fankeel/docs.json names. Nothing else was searched.');
        } else if (want && want.heading) {
            lines.push('no directory tree under ' + want.heading
                + ', the heading .fankeel/docs.json names, in ' + SIGNPOSTS.join(', ') + '.');
        } else {
