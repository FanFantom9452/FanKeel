...
const KEYS = {
    ...
    'dispatch.floor': { values: ['sonnet', 'opus', 'fable', 'haiku'], builtin: 'sonnet' },
    'judge.model': { values: ['sonnet', 'opus', 'fable', 'haiku'], builtin: 'fable' },
    ...
    'stage.agents': { values: ['false', 'true', 'all'], builtin: 'false' },
};
...
// The canonical seven, reused rather than retyped: `lib/stages.js` already
// requires this module at its own top level (`rulesFor`'s fallback reads
// `profile.read`), so requiring it back up here at module-load time would
// hand that file an empty object before this one finishes. A require inside
// the function defers it to call time, well after both modules have loaded —
// `parseStageAgents` only ever runs from `read` or `write`, at runtime.
function controlledStages() {
    return require('./stages.js').FULL_ROUTE;
}
...
// names, lowercased and re-ordered to `controlledStages()`'s own order
// regardless of what order or how many repeats they arrived in, so two
// profiles naming the same set always compare equal. An empty string reads
// as `false`: the station's "apply machine defaults" control round-trips an
// off value through `String([])`, which is empty.
function parseStageAgents(raw) {
    const s = String(raw).trim().toLowerCase();
    if (s === 'false' || s === '') return { value: [] };
    if (s === 'true') return { value: ['survey'] };
    const canon = controlledStages();
    if (s === 'all') return { value: canon.slice() };
    const names = s.split(',').map((n) => n.trim()).filter(Boolean);
    if (!names.length) return { error: 'stage.agents is one of: false, true, all, or a comma-separated list of: ' + canon.join(', ') };
    const unknown = names.find((n) => !canon.includes(n));
    if (unknown) return { error: 'stage.agents: not a stage: ' + unknown + '. One of: ' + canon.join(', ') };
...
module.exports = { KEYS, projectFile, machineFile, configDirOf, read, write, suggest, landClause, summary, parseValue };
