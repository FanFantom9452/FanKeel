...
// `docs/collisions.md` lists the gaps around it that are still real; the scope
// guard defaults to `ask` now, and its old off default had a different reason.
//
...
// Asking whether a directory is a repository is a normal step here, and paying a
// process to be told so is the expensive way to find out. `lib/tracked.js` owns
// that test — this file carried its own copy, wrapped in a try/catch for an
// `fs.existsSync` that is documented never to throw.
const { isRepo } = require('./tracked.js');
...
// An MCP write tool is covered wherever git reports it, and this reads only
// `git status --porcelain`. A write landing outside the repository is the part
// no test here can pin.
...
module.exports = { parsePorcelain, dirtyPaths, writtenSince, claimWrites };
