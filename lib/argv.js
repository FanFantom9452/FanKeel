...
function splitAroundVerb(argv, stringFlags, verbs) {
    let end = argv.length;
    while (end > 0) {
        // `--flag`, `--flag=value`, and a flag the table has never heard of all
        // stand alone. None of them is a value, so none of them is a word.
        if (argv[end - 1].startsWith('--')) { end -= 1; continue; }
