...
function splitAroundVerb(argv, stringFlags, verbs) {
    ...
    while (end > 0) {
        ...
        // `--root=x` spends no second token, so it is peeled alone. Only a flag
        // the table knows: an unrecognised dash in this position is the user's
        // word, which is the rule `freeText` already keeps one door over.
        // `tests/task.test.js` records `--force` as a note for that reason, while
        // `lib/guard.js` prints it as a real flag on `clear` — the string table
        // is the only line that satisfies both. A boolean spends no token, so it
        // can go on being read off the whole argv without swallowing anything.
        if (last.startsWith('--') && eq !== -1 && Object.hasOwn(stringFlags, last.slice(2, eq))) {
            end -= 1;
            continue;
        }
        ...
        const spends = before !== undefined && before.startsWith('--')
            ...
            && !isVerb(argv[end - 1], verbs);
        if (!spends) break;
        end -= 2;
    }

    const trailing = argv.slice(end);
    const { head, verb, text } = splitAtVerb(argv.slice(0, end), stringFlags, verbs);
    return { head: trailing.concat(head), verb, text };
}
