...
const { rulesFor, templateFor, initRules, INIT_TEMPLATE, normaliseRoute, positionIn, nextStage, FULL_ROUTE, CLASSES, controlFor, controlling } = require('./stages.js');
...
const SCRIPTS = { survey: named('survey.js'), map: named('map.js'), ledger: named('ledger.js'), todoCheck: named('todo-check.js'), docsCheck: named('docs-check.js'), docsAudit: named('docs-audit.js'), residue: named('residue.js'), orient: named('orient.js'), task: named('task.js') };
...
function renderBrainBrief(mine, root) {
    ...
    lines.push('  - You cannot run Workflow. Dispatch `fankeel:fankeel-reader` or `fankeel:fankeel-reviewer` with the Agent tool, at most four in one response, and open every path:line one cites before you keep it: this replaces "one workflow" below.');
