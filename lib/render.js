...
function rulesLines(data) {
    ...
    const rules = rulesFor(data && data.stage, Object.assign({ next, ponytail: ponytailLine() }, SCRIPTS));
    const lines = [''];
...
function render({ mine, others, now, root, launch, transcript, unclaimed }) {
...
function renderResume({ mine }) {
    ...
    return lines.concat(rulesLines(data)).join('\n');
}
