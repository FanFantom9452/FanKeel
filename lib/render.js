...
const SCRIPTS = { survey: SURVEY_SCRIPT, todoCheck: TODO_CHECK_SCRIPT, docsCheck: DOCS_CHECK_SCRIPT, docsAudit: DOCS_AUDIT_SCRIPT };
...
function render({ mine, others, now, root, launch, transcript }) {
    ...
    // through is a decision somebody made at the start, and a stage name alone
    // gives no way to tell `verify` is next from `land` is next.
    ...
    const lines = ['FANKEEL ACTIVE — ' + taskOf(data) + ' @ ' + stageOf(data)
        + (at ? '  (' + at.step + ' of ' + at.steps + ')' : '')];
    lines.push('route: ' + route.map((r) => (r === stageOf(data) ? '[' + r + ']' : r)).join(' → '));
    ...
    // The shape itself, after the rules that describe it. Indented so it reads as
    // a quoted skeleton rather than as more instructions.
...
module.exports = { render, renderBrief, RETURN_RULES, SCRIPTS, SURVEY_SCRIPT, TODO_CHECK_SCRIPT, DOCS_CHECK_SCRIPT, DOCS_AUDIT_SCRIPT };
