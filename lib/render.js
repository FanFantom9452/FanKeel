...
const { handoffPath, commitPath, answerPath } = require('./handoff.js');
...
function renderBrainBrief(mine, root, profile) {
    ...
    if (stage === 'build') {
        ...
        lines.push('  - You have no Edit. A task whose Dispatch line says in-session goes to an implementer on model `' + profile.values['dispatch.floor'] + '` like any other: send it the task\'s brief file.');
    }
