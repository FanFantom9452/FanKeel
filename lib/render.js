...
// Resolved from this file rather than passed in, so the rule names a path that
// works from whatever directory the session happens to be in.
...
function render({ mine, others, now }) {
    ...
    for (const rule of rulesFor(data && data.stage, { survey: SURVEY_SCRIPT })) lines.push('  - ' + rule);

    return lines.join('\n');
}

module.exports = { render, SURVEY_SCRIPT };
