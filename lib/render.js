...
function renderBrainBrief(mine, root) {
    ...
    if (stage === 'build') {
        ...
        lines.push('  - You cannot commit: `git commit` and `git add` are refused to you. When a task\'s implementer has returned and you have read what it changed, write ' + commit + ' — the paths that task owns one per line, a blank line, then the commit message — and return `commit ' + commit + '` and nothing else. The controller commits and messages you `<base>..<sha>`: that is the task\'s pinned range for its reviewer, and where the skill says to commit, this is it. Its paths are relative to the repository root. An error about your file (no blank line, no message, a path git refused): fix the file and ask again. An error about anything else (no repository, usage): the stage is blocked, so say so in the report. Return the report path only when the whole stage is done.');
    }
    if (stage === 'verify') lines.push('  - You cannot edit or restore a file. To apply a mutation, run the test and restore the file, send an implementer: it does all three, and you read what it returns.');
