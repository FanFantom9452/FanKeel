...
function rulesLines(data, profile) {
    ...
    const rules = rulesFor(data && data.stage, Object.assign({ next, profileLand: profileLib.landClause(values || {}) }, SCRIPTS), values);
    const lines = [''];
...
function renderBrief({ mine, agentType }) {
    ...
    if (String(agentType || '').replace(/^fankeel:/, '') === 'fankeel-judge') lines.push('  - Answer once. The parent will not message you again; what you return is the whole judgement, and it is filed verbatim under docs/judgements/.');
