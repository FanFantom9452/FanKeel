...
function renderBrainBrief(mine, root, profile) {
    ...
    if (stage === 'verify') {
        const floor = (profile && profile.values && profile.values['dispatch.floor']) || 'sonnet';
        lines.push('  - You cannot edit or restore a file. To apply a mutation, run the test and restore the file, send an implementer on model `' + floor + '`: it does all three, and you read what it returns.');
    }
