...
// The code half of the fortnightly pass belongs to a plugin, and the rule said
// so unconditionally: a session on a machine without ponytail was told every
// turn to run a command that is not there. `docs/decisions/fankeel-shell.md`
// argues for the mechanism that fixes it — use theirs when theirs is installed,
// say plainly when it is not — and this is where it finally runs.
//
// Never empty, for the reason `rulesLines` gives below: `substitute` skips a
// falsy value and would ship the raw token. The absent case is a sentence rather
// than nothing, because the reader still has to know the half is not covered.
//
// One small read per prompt, on a hook that already reads half a megabyte of
// transcript. Not worth a cache that could go stale on an install.
const ponytailLine = (env) => (has('ponytail', env)
    ? 'Alongside it: /ponytail-audit for code.'
    : 'Nothing installed here does the code half; say so rather than skipping it.');
...
function rulesLines(data, profile) {
...
function render({ mine, others, now, root, launch, transcript, unclaimed, profile }) {
    ...
    for (const line of rulesLines(data, profile)) lines.push(line);
...
module.exports = { render, renderInit, renderResume, renderCarry, renderBrief, ponytailLine, RETURN_RULES, SCRIPTS, PLUGIN_ROOT, PLUGIN_MARK, SURVEY_SCRIPT, TODO_CHECK_SCRIPT };
