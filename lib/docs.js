...
function roleOf(tree, rel) {
    ...
    if (!p) return null;
    if (isSignpost(p)) return 'reference';
    if (!tree) return null;
...
function projectRootsFor(registryRoot, paths) {
    ...
    const seen = new Set();
    const add = (rel) => {
        const full = rel ? path.join(registryRoot, rel) : registryRoot;
...
//   source_of_truth: <path>[, <path>...]   or   generated-by <script>
//
...
//
//   `source_of_truth` does two things, told apart by what the entry names, and
//   it is a comma list because a page usually has more than one subject. An
//   entry naming **code** is the page declaring what it is about — often the
//   only place it says so, since a page that shows its scripts in fenced blocks
//   names them nowhere a link or a code span can be read. An entry naming
//   **another document** settles a pair: two pages describing one file is only
//   a defect when neither of them defers to the other. Nothing collides, because
...
// The root files above, told apart from every other reference page. They point
// at what they link to rather than describing it — a `TODO.md` bullet names the
// file it is short of, and a `README.md` row names the directory it is sending
// somebody to. That is still worth checking for drift, which is why `roleOf`
// calls them reference; it is not worth reading one against another page, which
// is what the pairs pool uses this for.
const isSignpost = (rel) => {
    const p = String(rel || '').replace(/\\/g, '/').replace(/^\.\//, '');
    return Boolean(p) && !p.includes('/') && ROOT_REFERENCE.includes(p);
};

module.exports = {
    PRESETS, STATE_DIR, read, write, normalise, roleOf, detect, frontmatter, projectRootsFor,
