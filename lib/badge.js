...
function clearLead(claudeDir, sessionId) {
    return unlinkFlag(leadPath(claudeDir, sessionId));
}
...
function clearBadge(claudeDir, sessionId) {
    return unlinkFlag(badgePath(claudeDir, sessionId));
}
...
//
// The lead goes with the flag, the way `clearBadge` and `clearLead` are called
// together everywhere else. Removing only the flag left the rail TokenBar
// actually draws from behind, so a session thirty days gone kept a live line for
// ever — and the directory never emptied, so it was never removed either.
// `force` because most sessions have no lead, and a missing one is not a failure.
//
...
function pruneBadges(claudeDir, keepSessionId, maxAgeMs) {
    ...
    for (const name of names) {
        ...
        try {
            ...
            fs.rmSync(file);
            fs.rmSync(file + '.lead', { force: true });
            removed++;
