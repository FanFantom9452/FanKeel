        ...
        for (let i = 0; i < lines.length; i++) {
            for (const re of patterns) {
                ...
                if (isDoc(file)) {
                    ...
                    decls.push({ file, line: i + 1, text: lines[i].trim().slice(0, 100) });
                }
                break;
            }
        }
    }
    return { total: files.length, decls, docs, named };
}
...
function report(result, terms) {
    if (result === null) {
        return 'fankeel survey: not a git repository, so there is no tracked file list to read.\n'
             + 'Search by hand and say what you searched for.';
    }
    const { total, decls, docs, named } = result;
    const head = terms.length
        ? 'fankeel survey — ' + total + ' tracked files, matching: ' + terms.join(', ')
        : 'fankeel survey — ' + total + ' tracked files, everything declared';

    const lines = [head, ''];
    lines.push(...section('files whose name matches:', named, (f) => f));
    lines.push(...section('declarations:', decls, (d) => d.file + ':' + d.line + '  ' + d.text));
