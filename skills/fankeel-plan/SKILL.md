...
|---|---|
| Setup, config or docs with no task of their own | `lib/stages.js:273` folds them in on purpose (`Fold setup and docs into the task needing them`) — a task with no matching bullet was never meant to have one. |
| A `**Dispatch:**` line naming `sonnet` with no reason given | Only the tiers above the floor owe one — `lib/stages.js:276` (`and anything above it names why on that line`); `sonnet` needing no argument is the rule working. |
| A plan reviewer's return holding only two kinds of line | `lib/stages.js:275` names its whole charter (`returning only promises with no task and Files blocks that disagree with their task.`) — nothing else is in its scope, so nothing else missing is a gap. |
...
it costs and how to buy half of it back. Nothing else settles the line that way:
not a host, and not a tool's gate read as an instruction — the Agent tool has no
gate, and the one on the Workflow tool is the Workflow tool's own.
