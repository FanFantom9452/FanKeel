...
- 〔stage-agents〕受控 design 站遇上 `design.mockup`：brain 被告知前端 task 要出 mockup，但 `STAGE_AGENTS.design` 只有 reader 與 reviewer，寫不了頁面。補 implementer 路徑，還是把規則從 brain 的 brief 濾掉 — [lib/stages.js](lib/stages.js).
