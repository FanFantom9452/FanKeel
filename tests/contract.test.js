...
// Thirteen files carry the version and nothing kept them together: two manifests
// and one line of frontmatter in each of the eleven skills. A release that missed
// one left a skill announcing a version the plugin is not, which is the kind of
// wrong nobody reads carefully enough to catch — the number is right in twelve
// places.
//
...
test('every file that carries the version carries the same one', () => {
  ...
  assert.equal(found.size, 13, 'the count moved: ' + [...found.keys()].join(', '));
