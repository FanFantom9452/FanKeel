...
test('gather carries a session record\'s gates onto the session row', () => {
    // Mutation that reddens this: delete
    // `gates: Array.isArray(data.gates) ? data.gates : null` from the
    // `sessions.push({...})` in `gather()` — the row's `gates` comes back
    // undefined and the fallback has nothing to read. It reddens the test
    // below as well, which is the same field's other half: under
    // `node:assert/strict` an `undefined` fails that one's `assert.equal`
    // against `null` too.
...
// `undefined`. Not because of `serialize()` — its `sessions:` mapping lists
// its keys and `gates` is not one of them, so the field never reaches
// `station-data.js` at all. Because `scripts/station.js`'s `--json` writes
// the whole gather model, and there a null survives where an undefined key
// is dropped. Mutation that reddens this alone: make that same line's
// `: null` a `: undefined`.
